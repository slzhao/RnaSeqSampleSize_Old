...
powerDistribution<-est_power_distribution(n=65,f=0.01,rho=2,distributionObject="TCGA_READ",selectedGenes=selectedGenes,storeProcess=TRUE)
...
@
\end{itemize}
As a result, we use \Rfunction{est\_power\_distribution} function and find the estimated power is 0.81 for random genes, 0.77 for specified gene list, and 0.78 for genes in Glycolysis and Gluconeogenesis (pathway 00010) pathway.
...
est_power_distribution(n=65,f=0.01,rho=2,distributionObject=dataMatrixDistribution,repNumber=5)
@
