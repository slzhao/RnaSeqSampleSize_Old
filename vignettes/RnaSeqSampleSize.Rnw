...
A user friendly web interface for \Biocpkg{RnaSeqSampleSize} package is provided at \url{http://cqs.mc.vanderbilt.edu/shiny/RnaSeqSampleSize/}. Most of the functions in Examples section can be performed in this website.
...
@
\end{itemize}
As a result, we use \Rfunction{est\_power\_distribtuion} function and find the estimated power is 0.81 for random genes, 0.77 for specified gene list, and 0.78 for genes in Glycolysis and Gluconeogenesis (pathway 00010) pathway.
...
\begin{itemize}
  ...
  \item False discovery rate: 0.01;
\end{itemize}
We will use \Rfunction{est\_count\_dispersion} to estitamete the gene read count and dispersion distribution of user's prior dataset. And then \Rfunction{est\_power\_distribtuion} function will be used to estimate power.
