...
est_pvalue_store_get_power3_m<-Vectorize(est_pvalue_store_get_power3, vectorize.args=c("x1", "x0"))

est_power3<-function(n, w=1.0, rho=2.0, lambda0=5, phi_0=1, beta=0.2, alpha=0.05, error=0.001){
	mu0<-lambda0
	mu1<-mu0*(rho*w)
	phi_1<-phi_0
	...
	a<-outer(q1_l:q1_u, q0_l:q0_u, est_pvalue_store_get_power3_m,yy=yyStore) <= alpha
...
##' sample_size(m=vm, m1=vm1, power=vpower, f=vf, w=vw, rho=vrho, lambda0=vlambda0, phi_0=vphi_0)
sample_size<-function(m=10000, m1=100, power=0.8, f=0.1, w=1, rho=2, lambda0=5, phi_0=1,nTable=system.file("extdata", "nTable.csv", package = "RnaSeqSampleSize")){
	r1<-m1 * power
	beta<-1-power
...
#	require("ssanv")
	...
	n_near<-find_near_N(nTable,rho=rho,phi0=phi_0,lambda0=lambda0,f=f,m=m, m1=m1, power=power)
	if (length(n_near)!=0) {
...
find_near_N<-function(nTable,rho,phi0,lambda0,f,power,m,m1) {
	...
	if (as.character(rho_near)==as.character(rho) & as.character(phi0_near)==as.character(phi0) & as.character(lambda0_near)==as.character(lambda0) & as.character(f_near)==as.character(f) & as.character(power_near)==as.character(power) & as.character(m_near)==as.character(m) & as.character(m1_near)==as.character(m1)) {
